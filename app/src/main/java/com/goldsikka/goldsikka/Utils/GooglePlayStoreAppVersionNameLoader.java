    ...
    protected void onPostExecute(String string) {
        if (isAvailableInPlayStore == true) {
            ...
            if (currentVersion.equalsIgnoreCase(newVersion)) {
                ...
                Toast.makeText(mContext, "App Updated", Toast.LENGTH_LONG).show();
            } else {
                isVersionAvailabel = true;
            }
            mWsCallerVersionListener.onGetResponse(isVersionAvailabel);
        }else {
           // Toast.makeText(mContext, "Already Updated", Toast.LENGTH_SHORT).show();
           // Toast.makeText(mContext, newVersion, Toast.LENGTH_SHORT).show();
        }
    }
